...
const DOCUMENTED_GAPS = [
  ...
  { id: 'G13', area: 'T8 · Deploy', priority: 'done', resolved: true, title: '✅ Cloudflare LIVE — Worker deployed · D1/KV/R2 bound · client↔D1 sync flowing', detail: 'LIVE: Worker adeptio-hr-v245.pathom-bot.workers.dev deployed with D1·KV·R2 bindings; /api/health → {ok, stores:15}, 0 errors. D1 migrated AND receiving live client data (33 employees + payroll/leave/audit/db_platform; 14 of 15 stores replicate, db_identity custody-excluded). Client local-first + 30s replication to the edge is verified flowing.', action: 'Done — Worker live + client↔D1 sync verified via /api/health + /api/sync. Optional owner steps: wrangler secret put (SMTP/LINE/WA/SMS/PEPPER) + attach a custom domain.' },
