...
const DOCUMENTED_GAPS = [
  ...
  { id: 'G10', area: 'T0 · Mobile nav', priority: 'low', title: 'Mobile tab bar not filtered by hiddenScreens', detail: 'The flag-driven nav-hide filter is applied to the web rail only; mobile tabs are not yet filtered by FLAGS.hiddenScreens.', action: 'Apply FLAGS.hiddenScreens(persona) to the mobile tab bar too.' },
  { id: 'G11', area: 'T0 · Topbar', priority: 'low', title: 'Tier chip + set-tier buttons render while licensing is OFF', detail: 'The Essential/Pro tier chip and set-tier buttons still render (cosmetic no-op) when licensing is disabled.', action: 'Hide the tier chip / set-tier buttons behind LICENSE.enabled.' },
  { id: 'G12', area: 'T6 · Channels', priority: 'low', title: 'Message send is a stub (Email/Push/SMS/LINE/WhatsApp)', detail: 'HR Communication composer lights channel chips from MAIL.ready, but the actual send is a stub — no gateway dispatch (by design until the Worker is wired).', action: 'Wire send to the Worker /mail · /webhook endpoints once deployed (currently expected stub).' },
